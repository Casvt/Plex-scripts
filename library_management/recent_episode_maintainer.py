...
extract_last_watch: Callable[[TMedia], Union[int, None]] = lambda media: media.get("lastViewedAt")
extract_view_count: Callable[[TMedia], int] = lambda media: media.get("viewCount", 0)
extract_added_at: Callable[[TMedia], Union[int, None]] = lambda media: media.get("addedAt")
extract_size: Callable[[TMedia], Union[int, None]] = lambda media: media.get("Media", [{}])[0].get("Part", [{}])[0].get("size")
extract_res: Callable[[TMedia], Union[int, None]] = lambda media: RESOLUTIONS.get(media.get("Media", [{}])[0].get("videoResolution"))
extract_bitrate: Callable[[TMedia], Union[int, None]] = lambda media: media.get("Media", [{}])[0].get("bitrate")
extract_added_date: Callable[[TMedia], Union[datetime, None]] = lambda media: None if not "addedAt" in media else datetime.fromtimestamp(media["addedAt"])
