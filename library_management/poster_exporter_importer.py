            ...
            if len(self.artists) > 1:
                ...
                if len(self.albums) > 1:
                    if self.discs:
                        ...
                        raise ValueError(
                            "Can't give tracks for multiple albums")
                    ...
                    if len(self.discs) > 1 and self.tracks:
                        ...
                        # Tracks but no discs
                        raise ValueError(
                            "Can't give tracks without specifying a disc number")

                else:
                    if self.discs:
                        # Discs but no album
                        raise ValueError(
                            "Can't give disc numbers without specifying an album")

                    elif self.tracks:
                        # Tracks but no album
                        raise ValueError(
                            "Can't give track numbers without specifying an album")

            else:
                if self.albums:
                    # Albums but no artist
                    raise ValueError(
                        "Can't give albums without specifying an artist")

                elif self.discs:
                    # Discs but no artist
                    raise ValueError(
                        "Can't give disc numbers without specifying an artist")

                elif self.tracks:
                    # Tracks but no artist
                    raise ValueError(
                        "Can't give track numbers without specifying an artist")
        ...
        if lib['type'] == 'movie':
            ...
            for show in lib_output:
                ...
                for season in show_output["Children"].get("Metadata", []):
                    if (
                        lf.season_numbers
                        and not season['index'] in lf.season_numbers
                    ):
                        continue
                    ...
                    if "Metadata" not in season_output:
                        continue
            ...
            for artist in lib_output:
                ...
                for album in artist_output["Children"].get("Metadata", []):
                    ...
                    if "Metadata" not in album_output:
                        continue
                    ...
                    for track in album_output["Metadata"]:
                        ...
                        print(
                            f'          Disc {track["parentIndex"]} Track {track["index"]} - {track["title"]}')
                        yield track
