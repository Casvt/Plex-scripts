...
def _process_album(
    ...
    # If there are sub-albums, do them first
    # but also note first picture in album
    for image in album_output:
        if 'Media' not in image:
    ...
    if album_image:
        ...
        album_images[
            album_image['parentRatingKey']
        ] = album_image['Media'][0]['Part'][0]['file']
        ...
        # There isn't a picture in the album
        # so use the cover of the first sub-album
        for image in album_output:
            if 'Media' not in image:
                album_images[
                    image['parentRatingKey']
                ] = album_images[image['ratingKey']]
...
def first_photo_album_cover(
    ...
    Args:
        ...
        exclude_regex (List[str], optional): Regexes that, if matching,
        will exclude the album.
            Defaults to [].

        include_name (List[str], optional): Album names to only process.
            Defaults to [].

        include_regex (List[str], optional): Regexes that, if matching,
        will only process the album.
            Defaults to [].
