...
def keywords_to_genre(ssn, keywords: list, library_names: list, movie_names: list=[], series_names: list=[], skip_locked: bool=False):
	...
	for library_name in library_names:
		...
		for media in lib_output:
			...
			new_genres = [k for k in keywords if k in media_keywords and not k in media_genres.values()]
			...
			for i, g in list(media_genres.items()) + list(zip(range(len(media_genres), len(media_genres) + len(new_genres)), new_genres)):
				payload[f'genre[{i}].tag.tag'] = g
...
if __name__ == '__main__':
	...
	parser.add_argument('-k','--Keyword', type=str, help='Keyword that will be added to genre list if found in keyword list; allowed to give multiple times', action='append', required=True)
	parser.add_argument('-l','--LibraryName', type=str, help='Name of target library', action='append', required=True)
