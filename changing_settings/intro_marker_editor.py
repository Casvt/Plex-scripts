...
class LibraryFilter:
	...
	def __post_init__(self):
		if self.series is None and self.season_number is not None:
			raise ValueError("Can't give season number without specifying a series")

		if self.season_number is None and self.episode_number is not None:
			raise ValueError("Can't give episode number without specifying a season number")
		...
		if self.marker_start is not None:
			try:
				if ':' in self.marker_start:
					...
					self.marker_start_converted = int(split_time[0]) * 60000 + int(split_time[1]) * 1000
				else:
					self.marker_start_converted = int(self.marker_start)
			except ValueError:
				raise ValueError(f"Invalid value for marker start: {self.marker_start}")

		if self.marker_end is not None:
			try:
				if ':' in self.marker_end:
					split_time = self.marker_end.split(':')
					self.marker_end_converted = int(split_time[0]) * 60000 + int(split_time[1]) * 1000
				else:
					self.marker_end_converted = int(self.marker_end)
			except ValueError:
				raise ValueError(f"Invalid value for marker end: {self.marker_end}")

		if not None in (self.marker_start, self.marker_end):
...
def intro_marker_editor(
	...
	if action == Action.ADD and not (
		(lf.episode_number is not None or lf.movie) and mo.marker_start and mo.marker_end
	):
		raise ValueError("Adding a marker requires a specific media entry and a starting and ending time")

	if action == Action.REMOVE and not mo.marker_number:
		raise ValueError("Removing a marker requires it's marker number")

	if action == Action.EDIT and not (mo.marker_number and mo.marker_start and mo.marker_end):
		raise ValueError("Editing a marker requires it's marker number, a starting time and an ending time")

	if action == Action.SHIFT and (
		(lf.episode_number is not None or lf.season_number is not None or lf.series or lf.movie)
		and mo.marker_offset is None
	):
		raise ValueError("Shifting a marker requires a specific media entry and an offset to apply")
	...
	if action in (Action.EDIT, Action.REMOVE):
		...
		if action == Action.EDIT:
			cursor.execute(
				...
				(mo.marker_start_converted, mo.marker_end_converted, mo.marker_number)
			)
...
if __name__ == '__main__':
	...
	try:
		...
		parser.error("Run the script with root (a.k.a. administrator) privileges to make changes")
