		...
		for media in media_output.get("Media", []):
			for part in media["Part"]:
				...
				if dirname(part["file"]) == source_folder:
					...
					for extension in (splitext(part["file"])[1], ".srt", ".ass", ".nfo"):
					...
					dest_folder = join(target_folder, relpath(dirname(part["file"]), source_folder))
					makedirs(dirname(dest_folder), exist_ok=True)
					move(dirname(part["file"]), dest_folder)
					logging.info((f"Moved {dirname(part['file'])} to {dest_folder}"))
...
def move_file_after_watched(
	...
	if source_folder == target_folder:
		raise ValueError("Source folder and target folder cannot be the same folder")
