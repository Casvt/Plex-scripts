...
json_finder = compile(r'(?<=script id="__NEXT_DATA__" type="application/json">).*?(?=</script>)')
...
def keywords_to_genre(
	...
	for lib in sections:
		...
		for media in lib_output:
			...
			if skip_locked:
				if any(l['name'] == field and l['locked'] for l in media_output.get('Field', [])):
					continue
			...
			current_values = [g['tag'] for g in media_output.get(field.capitalize(), [])]
			...
			media_keywords = [e['rowTitle'] for e in imdb_json['props']['pageProps']['contentData']['section']['items'] if e['userVotingProps']['itemType'] == 'TitleKeyword']
			...
			for k in keywords:
				if '*' in k:
					...
					new_values += [r for r in result if not r in current_values]
				else:
					if k in media_keywords and not k in current_values:
						new_values.append(k)
			...
			payload = {
				...
				'id': media['ratingKey'],
				f'{field}.locked': '1',
				**{f'{field}[{c}].tag.tag': v for c, v in enumerate(current_values + new_values)}
			}
